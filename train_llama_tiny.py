...
model: nn.Module = Llama(
    vocab_size=args.vocab_size,
    num_layers=8,        # 从 12 改为 8（减少层数）
    num_heads=8,         # 从 12 改为 8（减少注意力头数）
    model_dim=512,       # 从 768 改为 512（减少隐藏维度）
    max_seq_len=max(args.train_seq_len, args.val_seq_len)
).cuda()

# model: nn.Module = Llama(
#     vocab_size=args.vocab_size,
#     num_layers=12,       # 从24改为12
#     num_heads=12,        # 从16改为12
#     model_dim=768,       # 从1024改为768
#     max_seq_len=max(args.train_seq_len, args.val_seq_len)
# ).cuda()
