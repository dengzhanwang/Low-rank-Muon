...
class GPT(nn.Module):
    ...
    def _forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        # 更新为8层的value embeddings结构（前3层和后3层使用value embeds）
        ve = [ve[0], ve[1], ve[2]] + [None] * (len(self.blocks) - 6) + [ve[0], ve[1], ve[2]]
        assert len(ve) == len(self.blocks)

        long_bm, short_bm = self.create_blockmasks(input_seq, sliding_window_num_blocks)
        # 更新block_masks为8层模式（每4层一个long_bm）
        block_masks = []
        for i in range(len(self.blocks)):
            if i % 4 == 0 or i == len(self.blocks) - 1:  # 第1、5、8层用long_bm
...
for step in range(train_steps + 1):
    ...
    # if step == 100:  # 每100次迭代
    if step % 100000 == 0 and step > 0:  # 每100次迭代
    # 只在主进程执行绘图
        if master_process:
