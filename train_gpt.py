...
def mm_op(x: Tensor, w: Tensor, x_s: float, w_s: float, grad_s: float) -> tuple[Tensor, Tensor, Tensor]:
    ...
    #     x_f8 = x.div(x_s).to(torch.float16)
    #     w_f8 = w.div(w_s).to(torch.float16)
    ...
    def impl(x: Tensor, w: Tensor):
        ...
        # Scale inputs and convert to FP16 (same as original)
        x_f8 = x.div(x_s).to(torch.float16)
        w_f8 = w.div(w_s).to(torch.float16)
        
        # Create scaling tensors (same as original)
        x_scale = x.new_tensor(x_s, dtype=torch.float32)
        w_scale = x.new_tensor(w_s, dtype=torch.float32)
        
        # Manual implementation of scaled matrix multiplication
        # 1. Apply scaling to the weights
        scaled_w = w_f8.T * w_scale
        
        # 2. Perform matrix multiplication
        out = torch.matmul(x_f8, scaled_w)
        
        # 3. Apply input scaling and convert to bfloat16
        out = out.mul(x_scale).to(torch.bfloat16)
...
def _(x: Tensor, w: Tensor, *_):
    ...
    assert x.is_contiguous() and w.is_contiguous()
    return x @ w.T, x.to(torch.float16), w.to(torch.float16)
...
def mm_backward_op(g: Tensor, x_f8: Tensor, w_f8: Tensor, x_s: float, w_s: float, grad_s: float) -> tuple[Tensor, Tensor]:
    ...
    def impl(grad: Tensor, x_f8: Tensor, w_f8: Tensor):
        ...
        # Convert gradients to FP16 and apply scaling
        grad_f8 = grad.div(grad_inv_s).to(torch.float16)
        
        # Compute grad_x using matmul
        # Apply scaling factors manually
        scaled_w = w_f8.T.contiguous().T * w_inv_s
        grad_x = torch.matmul(grad_f8, scaled_w).to(torch.bfloat16)
        
        # Compute grad_w using matmul
        # Apply scaling factors manually
        scaled_x = x_f8.T.contiguous() * x_inv_s
        scaled_grad = grad_f8.T.contiguous().T * grad_inv_s
        grad_w = torch.matmul(scaled_x, scaled_grad).T.to(torch.float32)
    ...
    #     w_inv_s = grad.new_tensor(w_s, dtype=torch.float32)
    #     grad_inv_s = grad.new_tensor(grad_s, dtype=torch.float32)
    #     grad_f8 = grad.div(grad_s).to(torch.float16)
...
optimizer1 = torch.optim.AdamW(adam_params, betas=(0.8, 0.95), eps=1e-10, fused=True)
