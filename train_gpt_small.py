...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, max_seq_len: int, layer_idx: int):
        super().__init__()
        self.attn = CausalSelfAttention(dim, num_heads, max_seq_len)  # 所有层都有注意力
        self.mlp = MLP(dim)
...
class GPT(nn.Module):
    ...
    def _forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        block_masks = [long_bm, short_bm, short_bm, short_bm, long_bm, short_bm, 
                       short_bm, short_bm, short_bm, short_bm, short_bm, long_bm]
...
def parse_args():
    ...
    # 数据参数
    parser.add_argument('--train_files', type=str, default="data/fineweb10Bedu/finewebedu_train_*.bin",
                       help='训练数据文件路径模式')
    parser.add_argument('--val_files', type=str, default="data/fineweb10Bedu/finewebedu_val_*.bin",
                       help='验证数据文件路径模式')
...
# begin logging
if master_process:
    run_id_full = f"small_{run_id:03d}_{uuid.uuid4()}"
