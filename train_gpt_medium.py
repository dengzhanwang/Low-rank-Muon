...
def zeropower_via_newtonschulz5(G: Tensor, use_Newton = 1, k = 200) -> Tensor:
    ...
    if G.size(-2) > G.size(-1):
        X = X.mT
    if G.size(-2) <= 1000 or G.size(-1) <= 1000:
    ...
    # Ensure spectral norm is at most 1
        ...
        for _ in range(50):  # 10 iterations
            A = X @ X.mT
        ...
        # n = G.size(-1)
        # # 步骤1: 生成随机投影矩阵
        # # Omega = torch.randn(n, k, device=G.device, dtype=G.dtype)  # n×k高斯矩阵
        # col_indices = torch.randperm(n, device=G.device)[:k]
        # # 步骤2: 构造Krylov子空间
        # Y = G[:, col_indices] 
        # Q, _ = torch.linalg.qr(Y)  # m×k正交基
        # # Q, _ = torch.linalg.qr(Y)
        # B = Q.T @ G
        
        # # 步骤4: 对小矩阵B做Newton-Schulz近似SVD
        # # a, b, c = self.newton_coeffs
        # X = B / (torch.norm(B) + 1e-7)
        # for a, b, c in [
        ...
        # # # 步骤5: 重建近似SVD
        # # U_approx = Q @ U_small  # m×k
        # # Vh_approx = U_small.T @ B  # k×n
...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        loss = checkpoint(self._forward,input_seq, target_seq, sliding_window_num_blocks)
        return loss
...
def parse_args():
    ...
    # 数据参数
    parser.add_argument('--train_files', type=str, default="data/fineweb100B/fineweb_train_*.bin",
                       help='训练数据文件路径模式')
    parser.add_argument('--val_files', type=str, default="data/fineweb100B/fineweb_val_*.bin",
                       help='验证数据文件路径模式')
    parser.add_argument('--val_tokens', type=int, default=10485760,
                       help='验证数据token数量')
    parser.add_argument('--train_seq_len', type=int, default=64 * 1024,
                       help='训练序列长度')
    parser.add_argument('--val_seq_len', type=int, default=4 * 64 * 1024,
                       help='验证序列长度')
    parser.add_argument('--optimizer', type=str, default='Muon',
                   choices=['Muon', 'Adam', 'SGD', 'RMSprop'],  # Example other optimizers
                   help='选择优化器 (默认: Muon)')
    ...
    # 特殊参数
    parser.add_argument('--use_Newton', type=lambda x: x.lower() == 'true', default=True,
...
# 使用方式
args = parse_args()  # 直接获取参数对象

# 示例：访问参数值
print(f"训练文件: {args.train_files}")
print(f"k: {args.k}")
print(f"使用Newton方法: {args.use_Newton}")
...
# begin logging
if master_process:
    run_id_full = f"medium_{run_id:03d}_{uuid.uuid4()}"
...
def _patched_trace_structured(name, metadata_fn, **kwargs):
    if name == "inductor_output_code":
        ...
        print0(f"inductor_output_code: {metadata_fn().get('filename', 'Unknown')}")  # 注意内部使用单引号
...
# optimizer2 = torch.optim.AdamW(hidden_matrix_params, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.0, fused=True)
if args.optimizer == "Adam":
    # optimizer2 = torch.optim.AdamW(hidden_matrix_params,lr=1e-4, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.01, fused=True)
    optimizer2 = torch.optim.Adam(hidden_matrix_params, lr=1e-4, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.01)
...
for step in range(train_steps + 1):
    ...
    # if step == 100:  # 每100次迭代
    if step % 100 == 0 and step > 0:  # 每100次迭代
    # 只在主进程执行绘图
        if master_process:
            ...
            for layer_idx, block in enumerate(model.blocks):
                ...
                # 获取QKV权重梯度
                qkv_grad = block.attn.qkvo_w.grad
                if qkv_grad is None:
                    continue
                
                # 分解Q、K、V分量
                grad_Q = qkv_grad[0].float()
                grad_K = qkv_grad[1].float()
                grad_V = qkv_grad[2].float()
                
                # 计算奇异值
                _, s_Q, _ = torch.svd(grad_Q)
                _, s_K, _ = torch.svd(grad_K)
                _, s_V, _ = torch.svd(grad_V)
                ...
                # 绘图
                ax = axs[row, col]
                ax.plot(s_Q, label='Q', alpha=0.7)
                ax.plot(s_K, label='K', alpha=0.7)
                ax.plot(s_V, label='V', alpha=0.7)
    ...
    for opt in optimizers:
        ...
        # print(f"Optimizer step time: {elapsed_time_ms:.3f} ms")
    # if dist.get_rank() == 0:
    #     print(f"Time: {(time.time()-start)*1000:.2f} ms")
    # null the gradients
    model.zero_grad(set_to_none=True)
